...
if "$cygwin" || "$msys" ; then
    ...
    set -- "$args0" "$args1" "$args2" "$args3" "$args4" "$args5" "$args6" "$args7" "$args8" "$args9" "$args10" "$args11" "$args12" "$args13" "$args14" "$args15" "$args16" "$args17" "$args18" "$args19"
fi

# Split up the JVM options only if this is not a cygwin properties file.
if [ -z "$CYGWIN_OPTS" ] ; then
